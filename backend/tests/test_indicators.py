...
def test_bollinger_reference():
    ...
    assert ind.std.iloc[2] == pytest.approx(1.0)
    assert ind.bb_upper.iloc[2] == pytest.approx(4.0)
    assert ind.bb_lower.iloc[2] == pytest.approx(0.0)


def test_ema_reference():
    # span=3 → alpha 0.5; ewm(adjust=False) of 1..6 ends at 5.03125
    ind = compute(RAMP, 3)
    assert ind.ema.iloc[-1] == pytest.approx(5.03125)


def test_window_clamped_to_two():
    ind = compute(pd.Series([1.0, 2.0, 3.0]), 1)
    assert ind.sma.iloc[1] == pytest.approx(1.5)   # rolling(2) mean of [1,2]


def test_constant_series_invariants():
    ind = compute(pd.Series([42.0] * 60), 5)
    assert (ind.std.dropna() == 0).all()
    assert ind.sma.dropna().to_numpy() == pytest.approx(42.0)
    assert ind.ema.to_numpy() == pytest.approx(42.0)
    assert ind.bb_upper.dropna().to_numpy() == pytest.approx(42.0)
