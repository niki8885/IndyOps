program portfolioopt
    use, intrinsic :: iso_fortran_env, only : input_unit, output_unit, error_unit
    use portfolio_mod, only : build_portfolio_report
    implicit none
    ...
    out = build_portfolio_report(buf)
    if (len(out) == 0) then
        write(error_unit, '(A)') 'portfolio-opt: empty or malformed request'
        stop 1
    end if
    write(output_unit, '(A)', advance = 'no') out
end program portfolioopt
